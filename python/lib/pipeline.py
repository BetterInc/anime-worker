...
from .hardware import (_compute_num_blocks_per_group, detect_vram,
                       estimate_model_memory_gb, system_ram_gb)
    ...
    for attr_name in dir(pipe):
        ...
        for submodule_name in ['encoder', 'decoder']:
            ...
            if shared.weight.data_ptr() != embed.weight.data_ptr():
                ...
                logger.info(f"  Fixed {attr_name} weight tying (shared -> {submodule_name}.embed_tokens)")
...
def setup_pipeline(config, model_config=None):
    ...
    dtype_str = model_config.get('model_dtype') or model_config.get('convert_model_dtype', 'bf16')
    dtype_map = {
        'fp8': torch.bfloat16,
        'fp16': torch.float16,
    ...
    if model_config.get('vae_class') == 'AutoencoderKLWan':
        ...
        vae_dtype = torch.float32 if model_config.get('vae_dtype') == 'float32' else dtype
    ...
    if best_gpu_free >= model_memory_gb + 8:
        ...
        logger.info(f"  Loaded entirely on GPU {best_gpu_id} ({best_gpu_free:.0f}GB free)")
        loaded = True
    ...
    if not loaded:
        ...
        try:
            ...
            logger.info(f"  Group offload on GPU {best_gpu_id} (num_blocks={num_blocks}, stream={use_stream})")
            loaded = True
        except Exception as e:
